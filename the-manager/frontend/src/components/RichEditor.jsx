...
export default function RichEditor({ value = '', onChange, onKeyDown: externalOnKeyDown, sx, ...textFieldProps }) {
  ...
  const btnSx = {
    p: '4px', borderRadius: 1, color: '#64748b',
  ...
  return (
    <Box>
      ...
      <Box sx={{
        ...
        <Box sx={{ width: 1, height: 14, bgcolor: '#cbd5e1', mx: 0.5 }} />
          ...
          if (e.key === 'Enter' && !e.shiftKey && !e.ctrlKey && !e.metaKey) {
            ...
            if (bulletMatch) {
              if (bulletMatch[2].trim() === '') {
                // Empty bullet — remove prefix and don't continue list
                e.preventDefault();
